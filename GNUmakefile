...
run-x86_64: store/edk2-ovmf $(IMAGE_NAME).hdd
	qemu-system-$(KARCH) \
		...
		-drive if=pflash,unit=1,format=raw,file=store/edk2-ovmf/ovmf-vars-$(KARCH).fd \
		-debugcon stdio \
		$(QEMUFLAGS)
...
store/limine:
	...
	git clone https://github.com/limine-bootloader/limine.git --branch=v10.x-binary --depth=1 store/limine
	$(MAKE) -C store/limine
...
$(IMAGE_NAME).iso: store/limine kernel
	...
	cp -v kernel/roanix-$(KARCH) iso_root/boot/roanix
	cp -v userland/distro-files/splash.jpg iso_root/boot/splash.jpg
...
ifeq ($(KARCH),x86_64)
	...
	mcopy -i $(IMAGE_NAME).hdd@@1M kernel/roanix-$(KARCH) ::/boot/roanix
...
clean:
	$(MAKE) -C kernel clean
	rm -rf iso_root roanix-*.iso roanix-*.hdd
